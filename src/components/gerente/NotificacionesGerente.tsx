...
export function NotificacionesGerente() {
  const [vistaActual, setVistaActual] = useState<'todas' | 'noLeidas' | 'criticas'>('noLeidas');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button onClick={handleMarcarTodasLeidas} variant="outline" disabled={noLeidas === 0}>
      ...
      <Tabs value={vistaActual} onValueChange={(v) => setVistaActual(v as any)}>
        ...
        <TabsContent value={vistaActual} className="mt-4 space-y-3">
              ...
              <Card key={notif.id} className={`transition-all ${!notif.leida ? 'border-l-4 border-l-orange-500 bg-orange-50/30' : ''}`}>
                <CardContent className="p-4">
                  <div className="flex items-start gap-4">
                    {/* Icono */}
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 ${getIconoBgColor(notif.tipo)}`}>
                      {notif.icono || <Bell className="w-6 h-6" />}
                    </div>
